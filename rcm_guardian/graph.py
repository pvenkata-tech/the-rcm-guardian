...
from rcm_guardian.db_uri import asyncpg_to_psycopg_conninfo
from rcm_guardian.metrics import (
    AUDITOR_CONFIDENCE,
    FINDING_TOTAL,
    GRAPH_NODE_SECONDS,
    HUMAN_REVIEW_TOTAL,
...
async def _timed_node(node: str, coro: Any) -> Any:
    t0 = time.perf_counter()
    try:
        return await coro
    finally:
        GRAPH_NODE_SECONDS.labels(node=node).observe(time.perf_counter() - t0)


def compile_rcm_graph(
    settings: Settings,
    rag: PayerRulesRAG,
    ...
    async def extractor(state: RCMGraphState) -> dict[str, Any]:
        ...
        return await _timed_node("extractor", work())
    ...
    async def rule_oracle(state: RCMGraphState) -> dict[str, Any]:
        ...
        return await _timed_node("rule_oracle", work())
    ...
    async def forensic_auditor(state: RCMGraphState) -> dict[str, Any]:
        async def work() -> dict[str, Any]:
            ...
            AUDITOR_CONFIDENCE.observe(confidence)
            for f in findings:
                fk = str(f.get("finding_kind") or "UNKNOWN")
                st = str(f.get("status") or "UNKNOWN")
                FINDING_TOTAL.labels(finding_kind=fk, status=st).inc()
            return {"audit_report": audit_report, "is_human_required": False}

        return await _timed_node("forensic_auditor", work())
...
async def create_rcm_graph_with_postgres(
    settings: Settings,
    rag: PayerRulesRAG,
) -> tuple[Any, AsyncConnectionPool]:
    """Compile the graph with AsyncPostgresSaver; caller must ``await pool.close()`` on shutdown."""
    uri = asyncpg_to_psycopg_conninfo(settings.database_url)
    ...
    graph = compile_rcm_graph(settings, rag, checkpointer)
    return graph, pool
...
__all__ = [
    ...
    "MemorySaver",
    "compile_rcm_graph",
    "create_rcm_graph_with_postgres",
]
